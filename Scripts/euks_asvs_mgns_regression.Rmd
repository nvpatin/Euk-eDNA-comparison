...
# Save r-squaraed values, max read counts, and model coefficients in separate vectors
...
p <- ggplot(slopersq, aes(x=slp, y=rsq, color=phy_unique_ordered)) + 
  ...
  geom_text_repel(aes(label=ifelse(abs(slp) > 5, as.character(phy_unique_ordered),'')),
            hjust=0.4, vjust=-1, show.legend=FALSE) +
...
p <- ggplot(slopersq, aes(x=slp, y=rsq, color=phy_unique_ordered)) + 
  geom_point(size=2, show.legend=TRUE) +
  geom_text(aes(label=ifelse(rsq > 0.25, as.character(phy_unique_ordered),'')),
            hjust=0.4, vjust=-1, show.legend=FALSE) +
  xlim(-5, 5) +
  theme_classic() +
  scale_color_manual(values=P50) +
...
ggsave(here("COI Comparison", "Phylum plots", "COI-mgns-rsq-slope-sub.png"),
       p,width=10, height=8, dpi=300, units="in")
```
