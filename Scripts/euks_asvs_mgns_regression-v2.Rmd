...
# remove missing and unknown phyla
taxa_phy <- taxa_phy[!taxa_phy$Phylum == '', ]
...
data_splits_COI <- split(taxa_phy_filt, taxa_phy_filt$Phylum)
```

## 18S
```{r split}
# Split the data into subsets based on Phylum
data_splits_18S <- split(taxa_phy_filt, taxa_phy_filt$Phylum)
```
...
```{r}
# create color palette
P50 = createPalette(40, c("#ff0000", "#00ff00", "#0000ff"))
P50 <- sortByHue(P50)
P50 <- as.vector(t(matrix(P50)))
names(P50) <- NULL

# Replace slope and r-squared NAs with 0s
pearsons_COI[is.na(pearsons_COI)] <- 0
pearsons_18S[is.na(pearsons_18S)] <- 0

pearsons_viol_COI <- as.data.frame(cbind(pearsons_COI, phy_unique_ordered))
pearsons_viol_18S <- as.data.frame(cbind(pearsons_18S, phy_unique_ordered))

slopersq$slp <- as.numeric(slopersq$slopes)
slopersq$rsq <- as.numeric(slopersq$r_squared_values)

p <- ggplot(pearsons_viol, aes()
